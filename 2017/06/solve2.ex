defmodule Solver do
  ...
  def try_distribute({buckets, found_patterns, steps}) do
    ...
    if Map.has_key?(found_patterns, buckets) do
      steps - found_patterns[buckets]
    else
      try_distribute({new_buckets, Map.put(found_patterns, buckets, steps), steps + 1})
    end
  end
end
...
IO.inspect Solver.try_distribute({buckets, %{}, 0})
