defmodule Infection do
  ...
  def step(inf) do
    inf =
    case (inf.map[inf.position] || ".") do
      "." ->
        inf |> turn_left() |> weaken()
      "W" ->
        inf |> infect()
      "#" ->
        inf |> turn_right() |> flag()
      "F" ->
        inf |> reverse() |> clear_infection()
    end
    inf |> move()
  end
  ...
  def reverse(inf) do
    new_vec = inf.vector + 2
    new_vec = if new_vec > 3, do: new_vec - 4, else: new_vec
    %Infection{inf | vector: new_vec}
  end
  ...
  def weaken(inf) do
    %Infection{inf | map: Map.put(inf.map, inf.position, "W")}
  end

  def flag(inf) do
    %Infection{inf | map: Map.put(inf.map, inf.position, "F")}
  end
