...
def find_north_pole(input)
  input.each do |line|
    if room_is_real(line)
      room_name = decode_room(line)
      if room_name.match(/north/)
        puts "#{decode_room(line)} - #{sector_value(line)}"
      end
    end
  end
end

def sector_value(line)
  sector_value = line.match(/(\d+)/)[1].to_i
  room_is_real(line) ? sector_value : 0
end

def room_is_real(line)
  room_code = line.gsub('-', '').match(/^([^0-9]+)/)[1]
  checksum = line.gsub('-', '').match(/\[(.+)\]/)[1]
  # puts "room_code: #{room_code}"
  # puts "checksum: #{checksum}"
  # puts "my checksum: #{checksum_for_code(room_code)}"
  checksum_for_code(room_code) == checksum
end

def checksum_for_code(code)
  letter_counts = Hash.new(0)
  code.split('').each do |char|
    letter_counts[char] += 1
  end
  checksum = letter_counts.keys.sort_by do |char|
    [0-letter_counts[char], char]
  end
  checksum.first(5).join
end

def decode_room(line)
  room_code = line.match(/^([a-z\-]+)/)[1].gsub('-', ' ')
  sector_value = sector_value(line)
  room_code.split('').map do |char|
    next char if char == ' '
    char_code = (((char.ord - 97) + sector_value) % 26) + 97
    char_code.chr
  end.join
end
